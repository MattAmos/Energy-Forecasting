...
def gen_error_histogram(
    set_name, model, period, baseline, energy_forecast_figure, slider_value, auto_state
):
    """
...
def deselect_auto(slider_value, energy_forecast_figure):
    ...
    if (
        energy_forecast_figure is not None
        and len(energy_forecast_figure["data"][0]["y"]) > 5
    ):
        return [""]
    return ["Auto"]
...
@app.callback(
    Output("model", "options"), Output("model", "value"), Input("dataset", "value")
)
...
@app.callback(
    Output("period", "options"), Output("period", "value"), Input("dataset", "value")
)
