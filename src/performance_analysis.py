...
def get_metrics(predictions, actual, cv, model_name):
    ...
    if cv:
        metrics = {
            model_name + "_RMSE": RMSE,
            model_name + "_R2": R2,
            model_name + "_MSE": MSE,
            model_name + "_MAE": MAE,
            model_name + "_MAPE": MAPE,
        }
    else:
...
def make_metrics_csvs(csv_directory, metrics, set_name, future, training):
    for model_name, metric_outputs in metrics.items():
        if not os.path.exists(
            csv_directory + "/" + set_name + "_metrics_" + str(future) + ".csv"
        ):
            metrics = pd.DataFrame({"Model": [], "Metric": [], "Value": []})
            metrics.loc[len(metrics)] = {
                "Model": model_name,
                "Metric": "RMSE",
                "Value": metric_outputs.get("RMSE"),
            }
            metrics.loc[len(metrics)] = {
                "Model": model_name,
                "Metric": "MAE",
                "Value": metric_outputs.get("MAE"),
            }
            metrics.loc[len(metrics)] = {
                "Model": model_name,
                "Metric": "MAPE",
                "Value": metric_outputs.get("MAPE"),
            }
            metrics.loc[len(metrics)] = {
                "Model": model_name,
                "Metric": "R2",
                "Value": metric_outputs.get("R2"),
            }
            if training:
                metrics.loc[len(metrics)] = {
                    "Model": model_name,
                    "Metric": "TIME",
                    "Value": metric_outputs.get("TIME"),
                }
            ...
            if model_name in metrics["Model"].values:
                metrics.loc[
                    (metrics["Model"] == model_name) & (metrics["Metric"] == "RMSE"),
                    "Value",
                ] = metric_outputs.get("RMSE")
                metrics.loc[
                    (metrics["Model"] == model_name) & (metrics["Metric"] == "MAE"),
                    "Value",
                ] = metric_outputs.get("MAE")
                metrics.loc[
                    (metrics["Model"] == model_name) & (metrics["Metric"] == "MAPE"),
                    "Value",
                ] = metric_outputs.get("MAPE")
                metrics.loc[
                    (metrics["Model"] == model_name) & (metrics["Metric"] == "R2"),
                    "Value",
                ] = metric_outputs.get("R2")
                if training:
                    metrics.loc[
                        (metrics["Model"] == model_name)
                        & (metrics["Metric"] == "TIME"),
                        "Value",
                    ] = metric_outputs.get("TIME")
            else:
                metrics.loc[len(metrics)] = {
                    "Model": model_name,
                    "Metric": "RMSE",
...
def make_csvs(
    csv_directory, predictions, y_test, pred_dates_test, set_name, future, model_name
):
    if not os.path.exists(
        csv_directory + "/" + set_name + "_performances_" + str(future) + ".csv"
    ):
        performances = pd.DataFrame(
            {"Date": pred_dates_test, "Actual": y_test, model_name: predictions}
        )
