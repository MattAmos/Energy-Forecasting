...
def tpot_cross_val_metrics(total_metrics, set_name, future):
    ...
    df.to_csv(
        csv_directory + "/" + set_name + "_cv_metrics_" + str(future) + ".csv",
        index=False,
    )


def tpot_make_csvs(
    csv_directory, predictions, y_test, pred_dates_test, set_name, future
):
    ...
    if not os.path.exists(
        ...
        performances = pd.DataFrame(
            {"Date": pred_dates_test, "Actual": y_test, "tpot": predictions}
        )
    ...
    if not os.path.exists(
        ...
        new_row = {
            "Model": ["tpot"],
            "RMSE": [metric_outputs.get("RMSE")],
            "R2": [metric_outputs.get("R2")],
            "MSE": [metric_outputs.get("MSE")],
            "MAE": [metric_outputs.get("MAE")],
...
def tpot_evaluate(
    future, set_name, X_train, y_train, pred_dates_test, X_test, y_test, y_scaler
):
    folder_path = os.getcwd()
